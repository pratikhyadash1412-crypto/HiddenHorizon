...
function DecisionSummary({ analysis }) {
  ...
  const normalizeImpact = (value) => {
    ...
    return Math.min(
      Math.max((number / 30) * 100, 0),
      100
    );
  };


  const overcrowdingScore =
    normalizeImpact(analysis.overcrowding_impact);

  const employmentScore =
    normalizeImpact(analysis.employment_impact);

  const localPurchaseScore =
    normalizeImpact(analysis.local_purchase_impact);

  const governmentProfitScore =
    normalizeImpact(analysis.government_profit_impact);

  const waterScore =
    normalizeImpact(analysis.water_saving);

  const wasteScore =
    normalizeImpact(analysis.waste_impact);

  const pollutionScore =
    normalizeImpact(analysis.pollution_impact);


  const accessibilityScore = Math.min(
    Math.max(
      Number(analysis.accessibility_score) || 0,
      0
    ),
    100
  );
  ...
  const localEconomyScore =
    (
      employmentScore +
      localPurchaseScore +
      governmentProfitScore
    ) / 3;
    ...
    <div className="decision-summary">
      ...
      <div className="decision-points">
        ...
        <div>
          ...
          <strong>
            {localEconomyScore >= 70
              ? "Strong"
              : localEconomyScore >= 40
                ? "Moderate"
                : "Limited"}
          </strong>
        </div>
      ...
      <div
        className={`government-decision ${decisionClass}`}
      >
    ...
    const loadLatestAnalysis = async () => {
      ...
      try {
        ...
        const response = await fetch(
          `${API_URL}/government/ai-analysis?_=${Date.now()}`,
          {
            method: "GET",
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to load AI analysis: ${response.status}`
          );
        }
         ...
         * IMPORTANT:
         *
         * The API may return multiple old analyses.
         *
         * Sort by ID so the newest analysis comes first.
         */
        const sortedAnalyses = [...data].sort(
          (a, b) =>
            Number(b.id || 0) -
            Number(a.id || 0)
        );
  ...
  return (
    <section className="ai-analysis-section">
      ...
      <div className="ai-analysis-heading">
        ...
        <p>
          AI-generated analysis of tourist redistribution
      ...
      {!loading && !error && !analysis && (
        <div className="ai-status">
          ...
          Run a redistribution simulation first.
        </div>
      )}
          ...
          >
            ...
            <div className="ai-route">
              ...
              <div className="ai-arrow">
                ...
                <small>
                  {analysis.visitor_shift_percentage}%
                  {" "}shift
                </small>
            ...
            <div className="ai-metrics-grid">
              ...
              <Metric
                label="Overcrowding Impact"
                value={analysis.overcrowding_impact}
              />

              <Metric
                label="Employment Impact"
                value={analysis.employment_impact}
              />

              <Metric
                label="Local Purchase Impact"
                value={analysis.local_purchase_impact}
              />

              <Metric
                label="Government Profit Impact"
                value={analysis.government_profit_impact}
              />

              <Metric
                label="Water Saving"
                value={analysis.water_saving}
              />

              <Metric
                label="Waste Impact"
                value={analysis.waste_impact}
              />

              <Metric
                label="Pollution Impact"
                value={analysis.pollution_impact}
              />

              <Metric
                label="Accessibility Score"
                value={analysis.accessibility_score}
              />
